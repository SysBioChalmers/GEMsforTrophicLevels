function solutions=randomSampling_vertGEM(model, nSamples, replaceBoundsWithInf,supressErrors,showProgress)
%   randomSampling_vertGEM
...
%   random set of reactions. Reactions that are closed (ub==lb==0) are not sampled.
%
...
%find goodRxns = model rxns that are not closed (ub=lb=0)
allmodelRxns=true(numel(model.rxns),1);
allmodelRxns=find(allmodelRxns);
closedRxnIDs=[];
for i = 1:length(model.rxns)
   if model.ub(i) == 0 && model.lb(i)==0 
      closedRxnIDs = [closedRxnIDs;i];
   end
end
goodRxns=setdiff(allmodelRxns, closedRxnIDs);

while counter<=nSamples
    if showProgress && rem(counter,200) == 0
